    ...
    if( ! empty( $jsons ) ) {
      ...
      foreach ( $jsons as $path ) {
        ...
        if( is_object( $object ) ) {
          ...
          echo '<h4 class="cs-icon-title">'. __( 'Error! Can not load json file.', 'themx' ) .'</h4>';
        }
...
if( ! function_exists( 'cs_set_icons' ) ) {
  function cs_set_icons() {
    ...
    echo '<div id="cs-icon-dialog" class="cs-dialog" title="'. __( 'Add Icon', 'themx' ) .'">';
    echo '<div class="cs-dialog-header cs-text-center"><input type="text" placeholder="'. __( 'Search a Icon...', 'themx' ) .'" class="cs-icon-search" /></div>';
    echo '<div class="cs-dialog-load"><div class="cs-icon-loading">'. __( 'Loading...', 'themx' ) .'</div></div>';
    echo '</div>';
